...
def get_source_position_xyz(head_pos_xyz, head_azim, src_azim, src_elev, src_dist):
    """
    Compute source xyz coordinates from head pose and source spherical coordinates.
    """
    head_pos_xyz = np.array(head_pos_xyz, dtype=float)
    return np.array(
        [
            src_dist
...
def normalize_angle_scalar(angle, lower=-180, upper=180):
    """
    Scalar wrapper around `normalize_angle`.
    """
    return float(normalize_angle(np.array([angle], dtype=float), lower=lower, upper=upper)[0])


def estimate_rt60_sabine(
    room_materials,
    room_dim_xyz,
    c=344.5,
    ...
    """
    Estimate RT60 from room geometry and material absorption using Sabine's formula.
    """
    room_dim_xyz = np.array(room_dim_xyz, dtype=float)
    x_len, y_len, z_len = room_dim_xyz
    surface_areas = np.array(
        [
            y_len * z_len,  # x=0 wall
            y_len * z_len,  # x=X wall
            x_len * z_len,  # y=0 wall
            x_len * z_len,  # y=Y wall
            x_len * y_len,  # floor
            x_len * y_len,  # ceiling
        ],
        dtype=float,
    ...
    freq = np.array(freq, dtype=float)
    alpha = np.zeros((6, len(freq)), dtype=float)
    for itr_surface, material in enumerate(room_materials):
        _, alpha_i = acoeff_hrtf(material=material, freq=freq)
        alpha[itr_surface, :] = alpha_i

    absorption_area = np.sum(surface_areas[:, np.newaxis] * alpha, axis=0)
    room_volume = x_len * y_len * z_len
    rt60_bands_sec = 0.161 * room_volume / np.maximum(absorption_area, 1e-8)
    # Mid-frequency average is commonly reported as an RT60 summary.
    mid_idx = [i for i, f in enumerate(freq) if f in [500, 1000, 2000]]
    if len(mid_idx) > 0:
        rt60_mid_sec = float(np.mean(rt60_bands_sec[mid_idx]))
    else:
        rt60_mid_sec = float(np.mean(rt60_bands_sec))

    return {
        "freq_hz": freq.tolist(),
        "rt60_bands_sec": rt60_bands_sec.tolist(),
        "rt60_mid_sec": rt60_mid_sec,
        "formula": "sabine",
        "sound_speed_m_per_s": float(c),
    }


def _fit_rt60_from_edc(edc_db, sr, db_hi, db_lo):
    """
    Fit decay slope between two dB limits and extrapolate to RT60.
    """
    idx = np.where((edc_db <= db_hi) & (edc_db >= db_lo))[0]
    if len(idx) < 2:
        return None
    t = idx / float(sr)
    slope_db_per_sec, intercept = np.polyfit(t, edc_db[idx], 1)
    if slope_db_per_sec >= 0:
        return None
    rt60_sec = -60.0 / slope_db_per_sec
    return {
        "rt60_sec": float(rt60_sec),
        "slope_db_per_sec": float(slope_db_per_sec),
...
def estimate_rt60_from_ir(brir, sr):
    """
    Estimate RT60 from a BRIR via Schroeder integration.

    Uses T30 when possible, then falls back to T20/T10.
    """
    brir = np.array(brir, dtype=float)
    if brir.ndim == 2:
        ir_mono = np.mean(brir, axis=1)
    else:
        ir_mono = brir

    energy = np.square(ir_mono)
    if np.all(energy == 0):
        return {
            "rt60_from_brir_sec": np.nan,
            "rt60_method": "unavailable",
            "dynamic_range_db": 0.0,
        }

    edc = np.cumsum(energy[::-1])[::-1]
    edc = edc / np.maximum(np.max(edc), 1e-16)
    edc_db = 10.0 * np.log10(np.maximum(edc, 1e-16))
    dynamic_range_db = float(np.abs(np.min(edc_db)))
    ...
    for method, db_hi, db_lo in candidates:
        fit = _fit_rt60_from_edc(edc_db, sr=sr, db_hi=db_hi, db_lo=db_lo)
        if fit is not None:
            return {
                "rt60_from_brir_sec": fit["rt60_sec"],
                "rt60_method": method,
                "dynamic_range_db": dynamic_range_db,
                "fit_slope_db_per_sec": fit["slope_db_per_sec"],
            }
...
def get_brir_meta(
    ...
    """
    Build metadata for a BRIR sample, including geometry, DOA, and RT60 estimates.
    """
    room_dim_xyz = np.array(room_dim_xyz, dtype=float)
    head_pos_xyz = np.array(head_pos_xyz, dtype=float)
    src_pos_xyz = get_source_position_xyz(
    ...
    x_len, y_len, z_len = room_dim_xyz
    room_volume_m3 = float(x_len * y_len * z_len)
    room_surface_area_m2 = float(2 * (x_len * y_len + x_len * z_len + y_len * z_len))

    doa_azim_head_deg = normalize_angle_scalar(src_azim)
    doa_elev_head_deg = float(src_elev)
    doa_azim_world_deg = normalize_angle_scalar(head_azim + src_azim)
    doa_elev_world_deg = float(src_elev)

    rt60_sabine = estimate_rt60_sabine(
    ...
    meta = {
        "room": {
            "room_dim_xyz_m": room_dim_xyz.tolist(),
            "room_materials": list(room_materials),
            "room_volume_m3": room_volume_m3,
            "room_surface_area_m2": room_surface_area_m2,
        },
        "listener": {
            "head_pos_xyz_m": head_pos_xyz.tolist(),
            "head_azim_deg": float(head_azim),
        },
        "source": {
            "src_pos_xyz_m": src_pos_xyz.tolist(),
            "src_dist_m": float(src_dist),
            "src_azim_deg_rel_head": float(src_azim),
            "src_elev_deg_rel_head": float(src_elev),
        },
        "doa": {
            "azim_deg_rel_head": doa_azim_head_deg,
            "elev_deg_rel_head": doa_elev_head_deg,
            "azim_deg_world": doa_azim_world_deg,
            "elev_deg_world": doa_elev_world_deg,
        },
        "rt60": {
            "sabine": rt60_sabine,
        },
    ...
    if brir is not None:
        meta["rt60"]["from_brir"] = estimate_rt60_from_ir(brir=brir, sr=sr)
...
def get_brir_and_meta(
    ...
    """
    Convenience wrapper that returns both BRIR and structured metadata.
    """
    brir = get_brir(
        room_materials=room_materials,
        room_dim_xyz=room_dim_xyz,
